    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center">
                ...
                <a class="btn btn-light" href="{{ route('dashboard.commande.create') }}">Ajouter une commande</a>
                    ...
                    <tbody>
                        ...
                        @foreach ($commandes as $commande)
                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="px-4 py-2 text-center">{{ $commande->firstname }}</td>
                                <td class="px-4 py-2 text-center">{{ $commande->lastname }}</td>
                                <td class="px-4 py-2 text-center">{{ $commande->email }}</td>
                                <td class="px-4 py-2 text-center">{{ $commande->dateDebut }}</td>
                                <td class="px-4 py-2 text-center">{{ $commande->dateFin }}</td>
                                <!-- Cellule de la ligne pour afficher le numéro de la commande -->
                                <td class="px-4 py-2 text-center">{{ $commande->user->firstname }}
                                    {{ $commande->user->lastname }}</td>
                                <!-- Cellule de la ligne pour afficher le nom complet de l'utilisateur associé à la commande -->
                                <td class="px-4 py-2 text-center">{{ $commande->vehicule->marque }}</td>
                                <!-- Affiche la marque du véhicule de la commande courante -->
                                <td class="px-4 py-2 text-center">{{ $commande->vehicule->model }}</td>
                                <!-- Affiche le modèle du véhicule de la commande courante -->
                                <td class="px-4 py-2 text-center">
                                    <a class="btn btn-light btn-block" role="button"
                                        href="{{ route('dashboard.commande.edit', ['id' => $commande->id]) }}">Modifier</a>
                                    <!-- Bouton pour modifier la commande courante -->
                                    <form action="{{ route('dashboard.commande.delete', ['id' => $commande->id]) }}"
                                        method="post">
                                        @csrf
